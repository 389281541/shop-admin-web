    ...
    <el-card shadow="never" style="margin-top: 15px">
      ...
      <div class="table-layout">
        ...
        <el-row>
          ...
          <el-col :span="6" class="table-cell-title">订单可得积分</el-col>
        </el-row>
        <el-row>
          <el-col :span="6" class="table-cell">{{order.deliveryCompany | formatNull}}</el-col>
          <el-col :span="6" class="table-cell">{{order.deliveryBillNo | formatNull}}</el-col>
          <el-col :span="6" class="table-cell">{{order.autoConfirmDay}}天</el-col>
          <el-col :span="6" class="table-cell">{{order.integrationAward}}</el-col>
        </el-row>
      </div>
      <div style="margin-top: 20px">
        <svg-icon icon-class="marker" style="color: #606266"></svg-icon>
...
<script>
import {getOrder, changeReceiverInfo, changeMoneyInfo, closeOrder, updateNote, deleteOrder} from '@/api/order'
import LogisticsDialog from '@/views/order/components/logisticsDialog'
  ...
  methods: {
    ...
    handleUpdateReceiverInfo () {
      this.$confirm('是否要修改收货信息?', '提示', {
        ...
        changeReceiverInfo(this.receiverInfo).then(response => {
    ...
    showUpdateMoneyDialog () {
      ...
      this.moneyInfo.deliverFee = this.order.deliverFee
      this.moneyInfo.promotionAmount = this.order.promotionAmount
    ...
    handleUpdateMoneyInfo () {
      this.$confirm('是否要修改费用信息?', '提示', {
        ...
        changeMoneyInfo(this.moneyInfo).then(response => {
    ...
    handleCloseOrder () {
      this.$confirm('是否要关闭?', '提示', {
        ...
        closeOrder({ids: [this.closeInfo.id], note: this.closeInfo.note}).then(response => {
    ...
    handleMarkOrder () {
      this.$confirm('是否要备注订单?', '提示', {
        ...
        updateNote({id: this.markInfo.id, note: this.markInfo.note}).then(response => {
