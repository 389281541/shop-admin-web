...
<script>
import {fetchFlashPromotionSessionList, updateStatus, deleteFlashPromotionSession, createFlashPromotionSession, updateFlashPromotionSession} from '@/api/flashPromotionSession'
...
export default {
  ...
  methods: {
    ...
    handleUpdate (index, row) {
      ...
      this.flashSession.startTime = row.startTime
      this.flashSession.endTime = row.endTime
    },
    handleDelete (index, row) {
      this.$confirm('是否要删除该时间段?', '提示', {
        ...
        deleteFlashPromotionSession(row.id).then(response => {
          this.$message({
            type: 'success',
            message: '删除成功!'
          })
    ...
    handleDialogConfirm () {
      this.$confirm('是否要确认?', '提示', {
        ...
        if (this.isEdit) {
          updateFlashPromotionSession(this.flashSession).then(response => {
            this.$message({
              message: '修改成功！',
              type: 'success'
            })
          ...
          createFlashPromotionSession(this.flashSession).then(response => {
            this.$message({
              message: '添加成功！',
              type: 'success'
            })
    ...
    getList () {
      ...
      fetchFlashPromotionSessionList({}).then(response => {
