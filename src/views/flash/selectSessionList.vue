...
<script>
import {fetchAvailableList} from '@/api/flashPromotionSession'
...
export default {
  ...
  methods: {
    handleShowRelation (index, row) {
      this.$router.push({path: '/activity/flashSpuList', query: {flashPromotionId: this.$route.query.flashPromotionId, flashPromotionSessionId: row.id}})
    },
    getList () {
      this.listLoading = true
      fetchAvailableList({id: this.$route.query.flashPromotionId}).then(response => {
