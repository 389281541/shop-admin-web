    ...
    <el-dialog
      ...
      <el-form :model="flashPromotion"
        ...
        <el-form-item label="活动标题：">
          <el-input v-model="flashPromotion.theme" style="width: 250px"></el-input>
        </el-form-item>
...
<script>
import {fetchFlashPromotionList, updateStatus, deleteFlashPromotion, createFlashPromotion, updateFlashPromotion} from '@/api/flashPromotion'
...
export default {
  ...
  methods: {
    ...
    handleShowSessionList () {
      this.$router.push({path: '/activity/sessionList'})
    },
    ...
    handleDialogConfirm () {
      this.$confirm('是否要确认?', '提示', {
        ...
        if (this.isEdit) {
          updateFlashPromotion(this.flashPromotion).then(response => {
            this.$message({
              message: '修改成功！',
              type: 'success'
            })
          ...
          createFlashPromotion(this.flashPromotion).then(response => {
            this.$message({
              message: '添加成功！',
              type: 'success'
            })
    ...
    handleSelectSession (index, row) {
      this.$router.push({path: '/activity/selectSessionList', query: {flashPromotionId: row.id}})
    },
    getList () {
      this.listLoading = true
      fetchFlashPromotionList(this.listQuery).then(response => {
